...
function readUsers($file) {
    ...
    if ($f) {
        while (!feof($f)) {
            ...
            if ($line === "") continue;
            $p = explode("|", $line, 3);
            if (count($p) === 3) $users[$p[0]] = ["password"=>$p[1],"avatar"=>$p[2]];
        }
        fclose($f);
    }
    return $users;
...
function writeUsers($file, $users) {
    ...
    if ($f) {
        foreach ($users as $u => $d)
            fwrite($f, $u."|".$d['password']."|".$d['avatar']."\n");
        fclose($f);
        return true;
    }
    return false;
...
if (isset($_POST['nick_degistir'])) {
    ...
    foreach (glob("comments/comments_*.txt") as $cf) {
        $lines = []; $changed = false;
        $f = fopen($cf, "r");
...
if (isset($_POST['sifre_degistir'])) {
    ...
    if (!empty($kaydedilen) && $kaydedilen !== md5($mevcut)) { header("Location: Profile.php?hata=sifre_yanlis"); exit(); }

    $users[$currentUser]['password'] = md5($yeni);
    writeUsers($usersFile, $users);
...
<nav class="category-nav">
    ...
    <div class="auth-nav">
        <a href="Profile.php" style="color:var(--accent);font-weight:900;text-decoration:none;">
            <i class="fas fa-user-circle"></i> <?= htmlspecialchars($currentUser) ?>
        </a>
    </div>
</nav>
...
<div class="profile-wrapper">
    ...
    <a href="Main.php" style="color:var(--accent);text-decoration:none;font-weight:700;">
        ← Ana Sayfaya Dön
    </a>
</div>
